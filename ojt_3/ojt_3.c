...
#include <string.h>                            // memset 함수를 사용하기 위한 헤더 파일
...
void fill_spiral_array(int **arr, int max_row, int max_col)
{
    ...
    while ((max_col > 0) && (max_row > 0))      // 채워야 할 행, 열이 남아 있을 때까지 반복
    {
        for (int i = 0; i < max_col; i++)       // 현재 행에서 열을 따라 이동하면서 값 채우기, target이 1이면 오른쪽, -1이면 왼쪽 이동
        {
        ...
        for (int i = 0; i < max_row; i++)       // 현재 열에서 행을 따라 이동하면서 값 채우기, target이 1이면 아래, -1이면 위로 이동
        {
...
int main(void)
{
    ...
    if (check_error)
    {
        ...
        for (int i = 0; i < check_index; i++) // 0부터 check_index-1 까지 반복
        {
            if (arr[i] != NULL)
            {
                free(arr[i]);
